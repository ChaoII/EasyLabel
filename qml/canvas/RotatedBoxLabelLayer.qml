            ...
            Shape {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        property int realHandlerWidth:  handlerWidth
                        property int realHandlerHeight: handlerHeight
            ...
            if (segmentationLabelLayer.drawStatus === CanvasEnums.Drawing) {
                ...
                if(segmentationLabelLayer.listModel.rowCount() >= 1){
                    if(!segmentationLabelLayer.shapeFinished){
                        segmentationLabelLayer.tempPoints.push(mousePosition)
                        ...
                        if(segmentationLabelLayer.tempPoints.length ===2){
                            // let p0 = segmentationLabelLayer.tempPoints[0]
                            // let p1 = mousePosition
                            // let rotation = QmlUtilsCpp.calculateAngleToRotation(p0, p1)
                            // let width = QmlUtilsCpp.calculateLength(p0,  p1)
                            // let polygonPoints = QmlUtilsCpp.rotationRectangleToPoints(p0.x,p0.y,width,1,rotation)
                            segmentationLabelLayer.listModel.appendPoint(last, mousePosition);
                            // segmentationLabelLayer.listModel.setProperty(last, "points",polygonPoints);
                        }
                        if(segmentationLabelLayer.tempPoints.length ===3){
                            let p0 = segmentationLabelLayer.tempPoints[0]
                            let p1 = segmentationLabelLayer.tempPoints[1]
                            let p2 = mousePosition
                            let rotation = QmlUtilsCpp.calculateAngleToRotation(p0, p1)
                            let width = QmlUtilsCpp.calculateLength(p0,  p1)
                            let height = QmlUtilsCpp.calculateLength(p1,  p2)
                            let polygonPoints = QmlUtilsCpp.rotationRectangleToPoints(p0.x,p0.y,width,height,rotation)
                            segmentationLabelLayer.listModel.setProperty(last, "points",polygonPoints);

                        }
                        segmentationLabelLayer.tempPoints.pop()
        ...
        onReleased: function(mouse) {
            if (mouse.button === Qt.LeftButton) {
                if (segmentationLabelLayer.drawStatus === CanvasEnums.Drawing) {
                    ...
                    segmentationLabelLayer.tempPoints.push(point)
                    if(segmentationLabelLayer.tempPoints.length===3){
                        segmentationLabelLayer.shapeFinished = true
                        segmentationLabelLayer.shapeFinished=[]
                    }
                    if(segmentationLabelLayer.shapeFinished){
                        segmentationLabelLayer.listModel.addItem(segmentationLabelLayer.currentLabelID, segmentationLabelLayer.tempPoints, segmentationLabelLayer.zOrder++, true)
