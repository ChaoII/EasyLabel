...
RotationBoxAnnotationModel::RotationBoxAnnotationModel(QObject *parent)
    : AnnotationModelBase{parent} {
}

int RotationBoxAnnotationModel::rowCount(const QModelIndex &parent) const {
    return parent.isValid() ? 0 : items_.size();
}

QVariant RotationBoxAnnotationModel::data(const QModelIndex &index,
                                           int role) const {
...
bool RotationBoxAnnotationModel::setData(const QModelIndex &index,
...
QHash<int, QByteArray> RotationBoxAnnotationModel::roleNames() const {
...
bool RotationBoxAnnotationModel::setProperty(int index,
...
QVariant RotationBoxAnnotationModel::getProperty(int index,
                                                  const QString &property) {
    if (index < 0 || index >= items_.size())
        return "";
    QModelIndex modelIndex = createIndex(index, 0);
...
void RotationBoxAnnotationModel::addItem(int lableID,
...
int RotationBoxAnnotationModel::getPointSize(int index) {
    if (index < 0 || index >= items_.size())
        return 0;
    return items_[index].points.size();
}

QVariantList RotationBoxAnnotationModel::getPoints(int index){
    if (index < 0 || index >= items_.size())
        return {};
    return items_[index].points;
}

void RotationBoxAnnotationModel::appendPoint(int index, const QPointF &point) {
    if (index < 0 || index >= items_.size())
        return;
    QModelIndex modelIndex = createIndex(index, 0);
...
void RotationBoxAnnotationModel::updatePoint(int index, int pointIndex, const QPointF& point){
    if (index < 0 || index >= items_.size())
        return;
    if (items_.empty())
        return;
...
void RotationBoxAnnotationModel::updateLastPoint(int index,
                                                  const QPointF &point) {
    if (index < 0 || index >= items_.size())
        return;
    if (items_.empty())
...
void RotationBoxAnnotationModel::moveShape(int index, const QPointF &dPoint) {
    if (index < 0 || index >= items_.size())
        return;
...
void RotationBoxAnnotationModel::popFrontPoint(int index) {
    if (index < 0 || index >= items_.size())
        return;
...
void RotationBoxAnnotationModel::popBackPoint(int index) {
    if (index < 0 || index >= items_.size())
        return;
...
void RotationBoxAnnotationModel::setSelected(int index, bool selected) {
    if (index < 0 || index >= items_.size())
        return;
...
void RotationBoxAnnotationModel::removeItem(int index) {
    if (index < 0 || index >= items_.size())
        return;
...
void RotationBoxAnnotationModel::clear() {
    if (items_.isEmpty())
        return;
...
int RotationBoxAnnotationModel::getLabelID(int index) {
    if (index < 0 || index >= items_.size())
        return -1;
    return items_[index].labelID;
}

void RotationBoxAnnotationModel::removeAllSelected() {
    for (int i = 0; i < items_.size(); i++) {
        setSelected(i, false);
    }
}

void RotationBoxAnnotationModel::setSingleSelected(int index) {
    if (index < 0 || index >= items_.size())
        return;
    removeAllSelected();
...
int RotationBoxAnnotationModel::getSelectedIndex(int x, int y) {
    for (int i = 0; i < items_.size(); i++) {
        auto &item = items_[i];
...
QJsonArray RotationBoxAnnotationModel::toJsonArray() const {
...
bool RotationBoxAnnotationModel::saveToFile(const QString &annotationFilePath,
                                             int annotationType,
...
bool RotationBoxAnnotationModel::loadFromFile(
...
void RotationBoxAnnotationModel::setLabelID(int index, int labelID) {
    if (index < 0 || index >= items_.size())
        return;
...
bool RotationBoxAnnotationModel::exportAnotation(
...
bool RotationBoxAnnotationModel::exportYoloAnnotation(
    const QString &exportDir, const QVector<QPair<QString, QString>> &dataSet,
