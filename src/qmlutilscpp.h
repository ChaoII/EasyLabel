...
public:
    ...
    Q_INVOKABLE static double calcateAngleToRotation(const QPointF &p0,
                                                     const QPointF &p1);
    Q_INVOKABLE static double calcateLength(const QPointF &p0, const QPointF &p1);

    Q_INVOKABLE static double distancePointToPoints(const QPointF &point0,
                                                    const QPointF &point1,
                                                    const QPointF &point2);

    Q_INVOKABLE static QVector<QPointF> rotatedRectCorners(const QRectF &rect, double rotation);

    Q_INVOKABLE static bool pointInRotatedRect(const QPointF& point, const QRectF& rect, double rotation) ;


    Q_INVOKABLE  static bool isPointInRect(const QRectF& rect, const QPointF &point);

    Q_INVOKABLE  static bool isPointLeftOfLine(const QVector<QPointF>& points);

    Q_INVOKABLE  static bool isPointLeftOfLineScreen(const QVector<QPointF>& points);
